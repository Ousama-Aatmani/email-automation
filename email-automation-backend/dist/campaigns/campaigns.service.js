...
let CampaignsService = class CampaignsService {
    ...
    async create(createCampaignDto) {
        const campaign = this.campaignsRepository.create(Object.assign(Object.assign({}, createCampaignDto), { status: campaign_entity_1.CampaignStatus.ACTIVE }));
        return this.campaignsRepository.save(campaign);
    }
    async findAll(status, startDate, endDate) {
        const where = {};
        if (status) {
            where.status = status;
        }
